...
title PokeTama V10.03 - Envoi vers Test-2
cd /d "%~dp0"

echo PokeTama V10.03 - envoi vers Moretro3D/Test-2
echo.
where git >nul 2>nul
if errorlevel 1 (
...
echo Preparation des fichiers V10.03...
git -c safe.directory="%CD%" add -A
git -c safe.directory="%CD%" diff --cached --quiet
if errorlevel 1 (
  git -c safe.directory="%CD%" commit -m "PokeTama V10.03 - presentation des arenes Kanto"
  if errorlevel 1 goto :connexion
)
...
echo Envoi de la V10.03 sans effacement ni force push...
