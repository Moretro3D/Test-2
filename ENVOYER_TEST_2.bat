...
title PokeTama V10.19 - Envoi vers Test-2
cd /d "%~dp0"

echo PokeTama V10.19 - envoi vers Moretro3D/Test-2
echo.
where git >nul 2>nul
if errorlevel 1 (
...
echo Preparation des fichiers V10.19...
git -c safe.directory="%CD%" add -A
git -c safe.directory="%CD%" diff --cached --quiet
if errorlevel 1 (
  git -c safe.directory="%CD%" commit -m "PokeTama V10.19 - six sols accueil pixel art"
  if errorlevel 1 goto :connexion
)
...
echo Envoi de la V10.19 sans effacement ni force push...
