...
title PokeTama V10.09 - Envoi vers Test-2
cd /d "%~dp0"

echo PokeTama V10.09 - envoi vers Moretro3D/Test-2
echo.
where git >nul 2>nul
if errorlevel 1 (
...
echo Preparation des fichiers V10.09...
git -c safe.directory="%CD%" add -A
git -c safe.directory="%CD%" diff --cached --quiet
if errorlevel 1 (
  git -c safe.directory="%CD%" commit -m "PokeTama V10.09 - correctifs Boite et Pokedex"
  if errorlevel 1 goto :connexion
)
...
echo Envoi de la V10.09 sans effacement ni force push...
