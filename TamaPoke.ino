...
#define FW_VERSION "1.46.94-moretro3d-v10.15-home-gym-cabin"
...
// Cabane pixel-art des arènes, dessinée sur une grille de 4 px pour garder
// des contours parfaitement nets sur l'écran AMOLED rond.
void drawHomeGymCabin(uint8_t phase, uint8_t biome) {
  const int x=338, y=150;
  uint16_t outline=(phase==3)?C565(0x12,0x16,0x28):C565(0x31,0x25,0x24);
  uint16_t wall=(phase==3)?C565(0x69,0x55,0x52):C565(0xb9,0x79,0x50);
  uint16_t wallHi=(phase==3)?C565(0x8a,0x70,0x62):C565(0xdf,0xa0,0x67);
  uint16_t roof=(phase==3)?C565(0x35,0x2d,0x4d):C565(0x8d,0x36,0x3f);
  uint16_t roofHi=(phase==3)?C565(0x54,0x46,0x69):C565(0xc8,0x50,0x4a);
  uint16_t glass=(phase==3)?C565(0xff,0xd6,0x58):C565(0x77,0xd5,0xe9);
  uint16_t door=(phase==3)?C565(0x2b,0x24,0x31):C565(0x5d,0x36,0x2b);

  gfx->fillRect(x+8,y+82,92,8,lerp565(BIOME_SOIL[biome<6?biome:0],outline,7,16));
  gfx->fillRect(x+70,y+10,14,28,outline);
  gfx->fillRect(x+74,y+6,14,8,outline);
  gfx->fillRect(x+80,y-5,8,8,(phase==3)?C565(0x74,0x7b,0x96):C565(0xd6,0xd9,0xdc));
  gfx->fillRect(x+88,y-13,8,8,(phase==3)?C565(0x5d,0x66,0x85):C565(0xea,0xec,0xee));

  // Toit en escalier et liseré de lumière.
  gfx->fillRect(x+36,y,32,4,outline);
  gfx->fillRect(x+28,y+4,48,4,outline);
  gfx->fillRect(x+20,y+8,64,4,outline);
  gfx->fillRect(x+12,y+12,80,4,outline);
  gfx->fillRect(x+4,y+16,96,8,outline);
  gfx->fillRect(x+12,y+20,80,8,roof);
  gfx->fillRect(x+20,y+16,64,4,roofHi);
  gfx->fillRect(x+28,y+12,48,4,roofHi);
  gfx->fillRect(x+36,y+8,32,4,roofHi);

  // Façade, poutres, porte et fenêtres éclairées la nuit.
  gfx->fillRect(x+14,y+28,76,56,outline);
  gfx->fillRect(x+20,y+32,64,48,wall);
  gfx->fillRect(x+24,y+36,56,5,wallHi);
  gfx->fillRect(x+20,y+52,64,4,outline);
  gfx->fillRect(x+46,y+32,5,48,outline);
  gfx->fillRect(x+55,y+56,22,24,door);
  gfx->fillRect(x+59,y+60,14,20,lerp565(door,wallHi,3,16));
  gfx->fillCircle(x+70,y+70,2,C565(0xf5,0xd4,0x63));
  gfx->fillRect(x+25,y+58,16,14,outline);
  gfx->fillRect(x+29,y+62,8,6,glass);
  gfx->fillRect(x+78,y+58,6,14,outline);
  gfx->fillRect(x+80,y+62,4,6,glass);

  // Enseigne-badge : repère visuel de l'accès aux huit arènes.
  gfx->fillRect(x+54,y+35,24,17,outline);
  gfx->fillRect(x+58,y+39,16,9,C565(0xe7,0xc5,0x55));
  gfx->fillRect(x+64,y+36,4,15,C565(0xff,0xe9,0x82));
  gfx->fillRect(x+58,y+42,16,3,C565(0xff,0xe9,0x82));

  if (biome==5) {
    uint16_t snow=C565(0xf1,0xf6,0xff);
    gfx->fillRect(x+4,y+16,24,4,snow);
    gfx->fillRect(x+76,y+16,24,4,snow);
    gfx->fillRect(x+10,y+82,88,4,snow);
  }
}
