...
#define FW_VERSION "1.46.101-moretro3d-v10.22-transparent-habitats"
...
void drawScene(uint8_t biome, uint32_t now, bool night) {
  ...
  // Efface totalement les anciens sols : le ciel se prolonge désormais sous
  // les nouveaux décors transparents, sans rectangle de couleur résiduel.
  gfx->fillRect(0,HORIZON,466,466-HORIZON,bot);

  // Remplace les anciens aplats par le décor détaillé associé au type.
  drawHomeHabitatGround(b);

  // Le compagnon est dessiné ensuite : l'arène reste bien dans le décor.
