...
WATER_SOURCE = ROOT / "assets" / "home_habitat_water_v10_21.png"
...
# Balanced safe inset, raised above the old horizon. Drawn at x2, every habitat
# is 360x180: wider than V10.21 while retaining its real transparent silhouette.
W, H = 180, 90
DISPLAY_X, DISPLAY_Y = 53, 210
...
for row in range(2):
    for col in range(3):
        ...
        # Transparent curved mask matching the safe interior of the round
        # display. The existing biome colour remains visible underneath.
        for py in range(H):
            sy = DISPLAY_Y + py * 2 + 1
            half = max(0.0, (226.0 ** 2 - (sy - 233.0) ** 2) ** 0.5)
            left, right = 233.0 - half + 6.0, 233.0 + half - 6.0
            for px in range(W):
                sx = DISPLAY_X + px * 2 + 1
